...
#################################################### Product Price End ######################################
  ...
  if($_REQUEST['id']=='')
  {  
    $obj->query("insert into $tbl_product set cat_id='$cat_id',subcat_id='$subcat_id',subsubcat_id='$subsubcat_id',brand_id='$brand_id',vendor_id='".$_SESSION['user_id']."',product_name='$product_name',slug='$product_slug',product_code='$product_code',short_description='$short_description',description='$description',meta_tags='$meta_tags',posted_date=now(),posted_by='".$_SESSION['user_id']."'",$debug=-1); //die;
    ...
    $updatequery=$obj->query("update $tbl_product set cat_id='$cat_id',subcat_id='$subcat_id',subsubcat_id='$subsubcat_id',brand_id='$brand_id',product_name='$product_name',slug='$product_slug',product_code='$product_code',short_description='$short_description',description='$description',meta_tags='$meta_tags',new_release='$new_release',posted_date=now() where id=".$_REQUEST['id'],$debug=-1); 
...
if($_REQUEST['id']!=''){
  $sql=$obj->query("select * from $tbl_product where id=".base64_decode(base64_decode(base64_decode($_REQUEST['id']))));
...
<h4><?php if($_REQUEST['id']!=''){?>Update <?php }else{?> Add <?php }?> Product</h4>
</div>
<div class="col-md-12">
...
<section class="content">
        <div class="box box-primary">
          <form name="productfrm" id="productfrm" method="POST" enctype="multipart/form-data" action="" onSubmit="return validate(this)">
            ...
            <input type="hidden" name="id" value="<?php echo base64_decode(base64_decode(base64_decode($_REQUEST['id'])));?>" />
            <div class="box-body">
              <div class="row">
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <select name="subsubcat_id" id="subsubcat_id" class="required form-control select2" >
                      <?php
                      if($_REQUEST['id']!=''){
                        $subsubcatSql = $obj->query("select * from $tbl_subsubcategory where status=1 and cat_id='".$result->cat_id."' and subcat_id='".$result->subcat_id."'");
                      while($subcatResult = $obj->fetchNextObject($subsubcatSql)){?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#subsubcat_id').change(function() {
      ...
      $.ajax({
        url:"ajax/getsubSubcategory.php",
        data:{subcat_id:subcat_id},
        beforeSend:function(){
        $("#subsubcat_id").html('<option value="">Select Sub Sub Category</option>');
        },
        success:function(data){
        $("#subsubcat_id").append(data);
        }
      })
     })
