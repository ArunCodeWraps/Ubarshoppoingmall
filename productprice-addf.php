...
//move_uploaded_file($_FILES['image_upload_file']['tmp_name'],"upload_images/model/".$fileNameNew);
  ...
  header("location: productprice-list.php?product_id=".base64_encode(base64_encode(base64_encode($_REQUEST['p_id']))));
  exit();
}      


if($_REQUEST['id']!=''){
  $sql=$obj->query("select * from $tbl_productprice where id=".base64_decode(base64_decode(base64_decode($_REQUEST['id']))));
...
<h4><?php if($_REQUEST['id']!=''){?>Update <?php }else{?> Add <?php }?> Product Price</h4>
</div>
<div class="col-md-12">
...
<section class="content">
        <div class="box box-primary">
          <form name="productPricefrm" id="productPricefrm" method="POST" enctype="multipart/form-data" action="" onsubmit="return validate(this)">
            ...
            <input type="hidden" name="p_id" id="p_id" value="<?php echo base64_decode(base64_decode(base64_decode($_REQUEST['product_id'])));?>" />
